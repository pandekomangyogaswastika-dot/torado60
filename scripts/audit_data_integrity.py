...
async def main():
    ...
    # stock_movements without outlet_id
    sm_no_outlet = await db.stock_movements.count_documents({"$or": [{"outlet_id": None}, {"outlet_id": {"$exists": False}}, {"outlet_id": ""}]})
    print(f"  stock_movements without outlet_id: {sm_no_outlet}")
    if sm_no_outlet:
        findings.append(f"{sm_no_outlet} stock_movements without outlet_id")
