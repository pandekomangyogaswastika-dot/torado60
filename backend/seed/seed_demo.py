...
Creates:
- 1 Group (Torado)
- 5 Brands (Altero, De La Sol, Calluna, Rucker Park, Bakkies) — 1 outlet each
...
- 5 sample outlet manager users (one per outlet)
...
ROLES = [
    ...
    {"code": "EXECUTIVE", "name": "Executive / Owner",
     ...
     "permissions": [
         ...
         "executive.budget.approve",  # Budget approval workflow
         "outlet_budget.read", "outlet_budget.set", "outlet_budget.monitor", "outlet_budget.approve_increase",  # Outlet Operational Budget
         ...
         "finance.budget.read",  # Read budget data
    ...
    {"code": "GM", "name": "General Manager",
     "permissions": [
         "executive.dashboard.read", "executive.drilldown.read", "executive.export",
         "executive.dashboard_view.save",
         "outlet_budget.read", "outlet_budget.monitor", "outlet_budget.approve_increase",  # Can monitor & approve increases
         "procurement.pr.read", "procurement.pr.approve", "procurement.pr.reject",
         "procurement.po.approve",
         "ai.chat.use", "ai.autocomplete.use", "ai.forecast.read", "ai.anomaly.read",
         "ai.exec_qa.use", "ai.vendor_recommend.use",
         "search.global.use",
    ...
    {"code": "FINANCE_MANAGER", "name": "Finance Manager",
     "permissions": [
         ...
         "finance.payment_request.read", "finance.payment_request.create", "finance.payment_request.submit",
         "finance.payment_request.approve", "finance.payment_request.mark_paid",
         ...
         "finance.budget.read", "finance.budget.create", "finance.budget.update", "finance.budget.delete",  # Budget management
         "finance.ar.read", "finance.ar.create", "finance.ar.update", "finance.ar.send", "finance.ar.receive",  # AR Ledger
         "finance.asset.read", "finance.asset.create", "finance.asset.update", "finance.asset.depreciate",  # Fixed Assets
    ...
    {"code": "FINANCE_STAFF", "name": "Finance Staff",
     "permissions": [
         ...
         "finance.payment_request.read", "finance.payment_request.create", "finance.payment_request.submit",
         "finance.journal_entry.read", "finance.journal_entry.create",
         "finance.budget.read", "finance.budget.create",  # Can create/read budgets
         "finance.ar.read",  # AR read only
    ...
    {"code": "PROCUREMENT_MANAGER", "name": "Procurement Manager",
     "permissions": [
         ...
         "procurement.view", "procurement.market_list.manage",
         "inventory.view",
         "ai.autocomplete.use", "ai.chat.use", "ai.vendor_recommend.use",
         "search.global.use",
     ], "is_system": True},
    {"code": "PROCUREMENT_STAFF", "name": "Procurement Staff",
     "permissions": [
         ...
         "procurement.view",
         "inventory.view",
         "ai.autocomplete.use", "ai.vendor_recommend.use", "search.global.use",
     ], "is_system": True},
    {"code": "INVENTORY_MANAGER", "name": "Inventory Manager",
     "permissions": [
         "inventory.balance.read", "inventory.movement.read", "inventory.view",
    ...
    {"code": "OUTLET_MANAGER", "name": "Outlet Manager",
     "permissions": [
         ...
         "outlet.kdo.create", "outlet.bdo.create", "outlet.fdo.create",
         "outlet.daily_close.execute", "outlet.opname.execute",
         "outlet.manager",
         "outlet_budget.read", "outlet_budget.request_increase",  # See own budget + request increase
         "inventory.balance.read", "inventory.movement.read", "inventory.valuation.read",
         "inventory.view",
         "procurement.pr.read", "procurement.pr.create", "procurement.view",
         "procurement.market_list.manage",
         "reservations.read", "reservations.create", "reservations.manage",
         "loyalty.read", "loyalty.transaction.create", "loyalty.redemption.create",
         "admin.loyalty.read", "admin.loyalty.analytics.read", "admin.loyalty.manage_customers",
         "vouchers.read", "vouchers.redeem",
         "crm.view", "crm.customer.read",
         "ai.autocomplete.use", "ai.categorize.use", "ai.ocr.use",
    ...
    {"code": "OUTLET_STAFF", "name": "Outlet Staff",
     "permissions": [
         "outlet.daily_sales.read", "outlet.petty_cash.read",
         "outlet.petty_cash.create", "outlet.kdo.create", "outlet.bdo.create", "outlet.fdo.create",
         "outlet.opname.execute",
         "outlet_budget.read",  # See budget read-only
         "inventory.balance.read", "inventory.view",
         "ai.autocomplete.use", "search.global.use",
     ], "is_system": True},
    {"code": "KITCHEN_STAFF", "name": "Kitchen Staff",
     "permissions": ["outlet.kdo.create", "inventory.view", "ai.autocomplete.use", "search.global.use"],
     "is_system": True},
    {"code": "BAR_STAFF", "name": "Bar Staff",
     "permissions": ["outlet.bdo.create", "inventory.view", "ai.autocomplete.use", "search.global.use"],
     "is_system": True},
    {"code": "FLOOR_STAFF", "name": "Floor Staff",
     "permissions": ["outlet.fdo.create", "inventory.view", "ai.autocomplete.use", "search.global.use"],
     "is_system": True},
...
async def seed():
    ...
    series = [
        ...
        ("PRN", "PRN", 4, "PRN-{YY}{MM}-{0000}"),  # Payment Run batch
    ]
    ...
    emp_idx = 1
    for outlet_code in ("ALT", "DLS", "CAL", "RKP", "BKK"):
    ...
    for code, name in [("ALT", "Altero"), ("DLS", "De La Sol"),
        ...
        await db.users.insert_one(doc({
            ...
            "default_outlet_id": outlets_map[code],  # auto-select single outlet
            "failed_login_count": 0, "locked_until": None,
            "mfa_enabled": False,
        }))
    ...
    print("  bkk.manager@torado.id    — Bakkies Outlet Manager")
    print()
    print("Group: Torado | Brands: 5 | Outlets: 5")
