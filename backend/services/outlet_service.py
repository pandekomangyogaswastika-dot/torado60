...
async def validate_daily_sales(id_: str, *, user: dict) -> dict:
    ...
    # Sprint C — Voucher consumption (idempotent, best-effort)
    try:
        from services.voucher_service import consume_voucher
        fresh = await db.daily_sales.find_one({"id": id_, "deleted_at": None})
        if fresh and fresh.get("voucher_code"):
            await consume_voucher(
                code=fresh["voucher_code"],
                daily_sales_id=id_,
                customer_phone=None,
            )
    except Exception as e:  # noqa: BLE001
        import logging as _logging
        _logging.getLogger("aurora.voucher").warning("voucher consumption failed for daily_sales %s: %s", id_, e)
    return await get_daily_sales(id_)
