...
ENTITY_QUERY_PROFILES = {
    ...
    "leave_request": {
        ...
        "status_field": "status",
        "amount_label": "Hari",
        "describe": lambda d: d.get("doc_no") or f"{d.get('employee_name','?')} - {d.get('leave_type','')}",
        "link": lambda _d: "/hr/leaves",
        "secondary": "employee_id",
    },
    "stock_transfer": {
        ...
        "amount_label": "Total Value",
        "describe": lambda d: d.get("doc_no") or (d.get("id", "")[:8]),
        "link": lambda _d: "/inventory/transfers",
        "secondary": "outlet_id",
    },
    "ar_invoice": {
        ...
        "link": lambda _d: "/finance/ar-invoices",
...
async def _build_queue_for_user(
    ...
    *,
    ...
    for et in types:
        ...
        async for d in db[col].find(q, {"id": 1, "status": 1, "created_at": 1, "outlet_id": 1, "deleted_at": 1, "_id": 0}).sort("created_at", -1).limit(500):
            entity = serialize(d)
            try:
