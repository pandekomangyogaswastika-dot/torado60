...
export default function MasterData({ hideEntityTabs = false } = {}) {
  ...
  return (
    <div className="space-y-4" data-testid="master-data-page">
      {/* Entity tabs (hidden when rendered inside MasterDataHub which provides its own tabs) */}
      {!hideEntityTabs && (
      ...
      {/* Table */}
      <div className="glass-card overflow-hidden" data-testid="master-table-card">
        <DataTable
          columns={schema.columns.map((c) => ({
            key: c,
            label: c.replace(/_/g, " "),
            sortable: true,
            render: (row) => renderCell(row[c], c),
          }))}
          rows={data}
          loading={loading}
          empty={<EmptyState title={`Belum ada ${schema.label.toLowerCase()}`} />}
          renderExpanded={(row) => (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-1.5 text-sm" data-testid={`master-detail-${row.id}`}>
              {schema.fields.map((f) => (
                <div key={f.key} className="flex items-center justify-between gap-3 border-b border-border/20 py-1">
                  <span className="text-xs text-muted-foreground">{f.label.replace(/\s*\*$/, "")}</span>
                  <span className="text-right">{renderCell(row[f.key], f.key)}</span>
                </div>
              ))}
            </div>
          )}
          ...
          <div className="px-5 py-3 border-t border-border/50 flex items-center justify-between text-xs text-muted-foreground">
            ...
            <div className="flex gap-2">
              <button disabled={page <= 1} onClick={() => setPage(p => p - 1)}
                      className="px-3 py-1 rounded-full glass-input disabled:opacity-50"
                      data-testid="master-prev">Prev</button>
              <span className="px-2 py-1">{page}/{totalPages}</span>
              <button disabled={page >= totalPages} onClick={() => setPage(p => p + 1)}
                      className="px-3 py-1 rounded-full glass-input disabled:opacity-50"
                      data-testid="master-next">Next</button>
            </div>
          </div>
        )}
      </div>
...
function EntityDialog({ entity, schema, editing, onClose, onSaved }) {
  ...
  return (
    <Dialog open={!!editing} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="glass-card max-w-lg max-h-[90vh] overflow-y-auto" data-testid="master-dialog">
        <DialogHeader>
    ...
    return (
      <div className="space-y-1.5">
        ...
        <SimpleSelect
          ...
          options={[{ value: "", label: "-- pilih --" }, ...field.options.map(o => ({ value: o.v, label: o.l }))]}
          placeholder="-- pilih --"
